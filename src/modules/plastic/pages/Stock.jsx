 ...
 * purchase list (void). Buying is occasional, so it lives under Settings.
 */
  ...
  return (
    <div className="max-w-lg mx-auto p-4 space-y-4">
      ...
      {stock.lowItems.length > 0 && (
        <Card className="p-3 !bg-amber/10 !border-amber/30 text-sm text-amber">
      ...
      {stock.all.map(item => (
        <Card key={item.id} className="p-4">
          <div className="flex items-center justify-between">
            <span className="font-bold text-chrome">{item.name}</span>
            {item.low && <span className="text-xs bg-signal-red/15 text-signal-red font-bold px-2 py-0.5 rounded-full">LOW</span>}
          </div>
          <div className="mt-2 flex items-end justify-between">
            <div>
              <div className={`font-mono tnum text-3xl font-bold ${item.stock < 0 ? 'text-signal-red' : 'text-amber'}`}>{fmtNum(item.stock)} <span className="text-base text-muted">{item.unit}</span></div>
              <div className="text-xs text-muted mt-0.5">in stock</div>
            </div>
            <div className="text-right text-xs text-muted">
              ...
              {item.latest > 0 && item.latest !== item.masterRate && (
                <button onClick={() => applyLatestRate(item)} className="mt-1 text-amber font-semibold underline">use latest as rate</button>
              )}
            </div>
          </div>
          <div className="mt-3 flex items-center gap-2">
            <span className="text-xs text-muted">Alert below</span>
            <span className="w-20">
      ...
      {!adding ? (
        ...
        <Card className="p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div><span className="text-xs text-muted">Date</span><DateInput value={date} onChange={e => setDate(e.target.value)} className="mt-1" /></div>
            <div><span className="text-xs text-muted">Material</span><Select className="mt-1" value={kind} onChange={e => onKind(e.target.value)} options={[{ value: 'compound', label: 'Compound' }, { value: 'nut', label: 'Nut / insert' }]} /></div>
          </div>
          <div><span className="text-xs text-muted">Item</span><Select className="mt-1" value={materialId} onChange={e => setMaterialId(e.target.value)} options={matOpts} /></div>
          <div className="grid grid-cols-2 gap-2">
            <div><span className="text-xs text-muted">Quantity ({kind === 'nut' ? 'pcs' : 'kg'})</span><NumberInput value={qty} onChange={e => setQty(e.target.value)} placeholder="0" className="mt-1" /></div>
            <div><span className="text-xs text-muted">Lot rate (₹/{kind === 'nut' ? 'pc' : 'kg'})</span><NumberInput value={rate} onChange={e => setRate(e.target.value)} placeholder="0" className="mt-1" /></div>
          </div>
          <div className="grid grid-cols-2 gap-2">
            <div><span className="text-xs text-muted">Supplier</span><TextInput value={supplier} onChange={e => setSupplier(e.target.value)} className="mt-1" /></div>
            <div><span className="text-xs text-muted">Invoice #</span><TextInput value={invoice} onChange={e => setInvoice(e.target.value)} className="mt-1" /></div>
          </div>
          {Number(qty) > 0 && Number(rate) > 0 && (
            <div className="text-center text-sm text-muted">Amount: <b className="text-amber">₹{fmtNum(Number(qty) * Number(rate))}</b></div>
          )}
          <div className="flex gap-2">
            <button onClick={() => setAdding(false)} className="flex-1 border border-hairline rounded-xl py-2.5 text-sm font-semibold text-muted">Cancel</button>
      ...
      <Card className="p-4">
        ...
        <div className="mt-2 divide-y divide-hairline">
          {recent.length === 0 && <p className="text-sm text-muted">No purchases yet. Record one to start tracking stock.</p>}
          {recent.map(p => {
            ...
            return (
              <div key={p.id} className={`py-2 flex items-center justify-between text-sm ${p.voided ? 'opacity-40 line-through' : ''}`}>
                <div>
                  <div className="font-semibold text-chrome">{name}</div>
                  <div className="text-xs text-muted">{fmtDate(p.date)} · {fmtNum(p.qty)} {p.kind === 'nut' ? 'pcs' : 'kg'} @ ₹{fmtNum(p.rate)}{p.supplier ? ` · ${p.supplier}` : ''}</div>
                </div>
                <div className="flex items-center gap-3">
                  <span className="font-mono text-chrome">₹{fmtNum((Number(p.qty) || 0) * (Number(p.rate) || 0))}</span>
                  {!p.voided && (
                    <button onClick={() => {
                    ...
                    }} className="text-xs text-signal-red font-semibold">Void</button>
                  )}
                </div>
              </div>
            )
