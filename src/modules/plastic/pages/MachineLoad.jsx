...
const VERDICT = {
  buy:            { label: '✅ BUY — volume justifies a machine', cls: '!bg-signal-green/10 !border-signal-green/30 text-signal-green' },
  'getting-close':{ label: '🟡 GETTING CLOSE — keep watching', cls: '!bg-amber/10 !border-amber/30 text-amber' },
  outsource:      { label: '⏸️ KEEP OUTSOURCING', cls: '!bg-graphite text-chrome' },
  nodata:         { label: 'ℹ️ Not enough data yet', cls: '!bg-graphite text-muted' },
}
  ...
  return (
    <div className="max-w-lg mx-auto p-4 space-y-3">
      ...
      <Card className="p-4 space-y-3">
        ...
        <div className="text-xs text-muted">≈ {fmtNum(m.monthly.hours)} machine-hrs/month of work vs {fmtNum(m.capacity.oneShiftHrs)} hrs (1 shift) available.</div>
      </Card>
      ...
      <Card className="p-4 space-y-2">
        ...
        {m.perProduct.length > 0 && (
          <div className="pt-1 border-t border-hairline text-xs text-muted">
      ...
      <Card className="p-4 space-y-2">
        ...
        <div className="pt-1 border-t border-hairline text-xs text-muted space-y-0.5">
          ...
          <div className="flex justify-between font-semibold text-muted"><span>Total in-house/month</span><span className="font-mono">₹{fmtNum(m.economics.monthlyTotal)}</span></div>
        </div>
        ...
        <div className="text-[11px] text-muted pt-1">Material + nut cost the same either way — only the conversion (machine + labour + power) differs. Set your real machine/operator/power figures in config to sharpen this.</div>
      </Card>
    </div>
  )
}
...
function Bar({ pct }) {
  return (
    <div className="h-3 bg-hairline rounded-full overflow-hidden">
      <div className={`h-full ${pct >= 80 ? 'bg-signal-green' : pct >= 50 ? 'bg-amber' : 'bg-muted'}`}
